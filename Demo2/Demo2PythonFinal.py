...
def writeMessage(angle,distance,detected,detectCount):
    if(detected == False):
        try:
        ...
        if(detectCount == 0):
            ...
            print("sent angle")
            try:
                bus.write_i2c_block_data(address, 3, dataInAscii)
            except OSError:
                print("I2C Error.")
...
while(True):
    ...
    if not len(corners) > 0:
        ...
        if(searchCount == 0):
            ...
            writeMessage(angle,finalDist,False,detectCount)
            detectCount = 0
            searchCount+=1
    ...
    # Calculate the position variables
        ...
        if detectCount == 0:
            #print(detectCount)
            writeMessage(angle,finalDist,True, detectCount)
            detectCount+=1
        
            
        #print(prevAngle)
        #print(angle)
        if((abs(prevAngle - angle) <= 0.1)):
